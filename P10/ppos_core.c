...
void initializeMainTask()
{
    ...
    mainTask_global.preemption_blocker = 0;
...
void ppos_init()
{
    ...
    dispatcherTask_global.preemption_blocker = 1;
    initialize_handler();
    initialize_timer();
              ...
              void *arg)                  // argumentos para a tarefa
{
    ...
    task_setprio(task, 0);
    // Caso não seja o dispatcher, é uma tarefa de usuário e pod ser preemptada por tempo
    if (idCounter_global != 1)
    {
        task_set_type(task, USER_TASK);
        task->preemption_blocker = 0;
    }
...
int task_switch(task_t *task)
{
    ...
    if (task == &dispatcherTask_global && currentTask_global == &dispatcherTask_global )
    {
        return -1;
    }
...
void handling(int signum)
{
    ...
    if (currentTask_global->status == SYSTEM_TASK || currentTask_global->preemption_blocker == 1)
        return;
...
{
    currentTask_global->preemption_blocker = 1;
    task_t *taskAux;
    ...
    currentTask_global->preemption_blocker = 0;
